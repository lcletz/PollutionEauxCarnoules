...
```
```{r}
worst_pred <- append(worst_pred, c("O2_lag_1","Cobal","Zinc","Temperature","pH","Conductivité"))
...
```{r}
worst_pred <- append(worst_pred, c("Conductivité","Arsenic_Total_lag_1","Zinc_lag_1"))    
...
```{r}
worst_pred <- append(worst_pred, c("Conductivité","Antimoine_lag_1","Nickel"))    
...
The most significant predictors are, by increasing order of significance: Cadmium, monthly_rainfall_lag_1, Plomb_lag_1, monthly_rainfall, Temperature, Cobalt, Nickel, Temperature_lag_1, Sulfate, Antimoine_lag_1, Conductivité and Arsenic_Total
...
```{r}
worst_pred <- append(worst_pred, c("Temperature","Conductivité","O2","Sulfate","Nickel",
...
summary(fit$gam)
```
Here are the significant predictors, by increasing order of significance: Thallium, Cuivre, O2, Ion_Ferreux, Sulfate, pH, Aluminium, Zinc and Cadmium.

There is no obvious link between arsenic and rain, but we could include monthly_rainfall_lag_4 since it's the last predictor that got removed to enhance the BIC. So, there might be a slight link between arsenic and rainfalls 4 months earlier. Arsenic is mostly explained by other chemicals.
...
```{r}
worst_pred <- append(worst_pred, c("Temperature","Sulfate","Zinc","Cobalt","monthly_rainfall_lag_1"))    
...
Let's keep in mind that there is indeed a strong link between rainfall (instant and 3 months earlier) and lead. Lead is also linked to many chemicals.
...
The most significant predictors are, by increasing order of significance: monthly_rainfall, monthly_rainfall_lag_1, monthly_rainfall_lag_4, Aluminium, Arsenic_Total, Sulfate.

There is a clear link between zinc and rain at almost every delay. Rainfall might have to do with its dilution.
...
Let's keep in mind that there is indeed a link between rainfall at all hour and copper. 
...
The most significant predictors are, by increasing order of significance: Arsenic_Total, O2, Plomb and Cuivre. This emphasizes the previous observations.

# Conclusion

Rainfall, whether that be at time T, T-1, T-2, T-3 or T-4, has a significant impact on chemicals' concentrations. 
That might still be the case today and, since it seems to be raining more often and in bigger quantity, those chemicals have a high probability of being mor concentrated.
...
#species <- data_complete[c(7:18,23:27)]
#env <- data_complete[c(2:6,19:22)]
species <- as.matrix(abs(data_complete[c(6:15)]))
env <- as.matrix(data_complete[c(2:5)])
```
...
plt.cc(prem.cca, d1 = 1, d2 = 4, var.label = TRUE, ind.names = data_complete$Dates2)
```
All variables are far from the sphere and evenly scattered...
...
library(FactoMineR)
g <- ggpairs(
  data = data_complete[, c(2,16:19,6:15)],
  lower = list(continuous = custom_gam_plot, combo = 'facetdensity'),
  upper = list(continuous = wrap('cor', size = 3, hjust = 0.8))
)
```
...
```{r, warning=F}
g[7,1] ; g[7,2] ; g[7,3] ; g[7,4] ; g[7,5]
```

```{r, warning=F}
g[8,1] ; g[8,2] ; g[8,3] ; g[8,4] ; g[8,5]
```

```{r, warning=F}
g[9,1] ; g[9,2] ; g[9,3] ; g[9,4] ; g[9,5]
```

```{r, warning=F}
g[10,1] ; g[10,2] ; g[10,3] ; g[10,4] ; g[10,5]
```

```{r, warning=F}
g[11,1] ; g[11,2] ; g[11,3] ; g[11,4] ; g[11,5]
```

```{r, warning=F}
g[12,1] ; g[12,2] ; g[12,3] ; g[12,4] ; g[12,5]
```

```{r, warning=F}
g[13,1] ; g[13,2] ; g[13,3] ; g[13,4] ; g[13,5]
```

```{r, warning=F}
g[14,1] ; g[14,2] ; g[14,3] ; g[14,4] ; g[14,5]
```

```{r, warning=F}
g[15,1] ; g[15,2] ; g[15,3] ; g[15,4] ; g[15,5]
```
...
```{r}
var_names <- rownames(pca$var$coord)
custom_groups <- ifelse(var_names %in% var_names[c(1:4,15:18)], 
                        "Élements physico-chimiques", 
                        "Éléments traces (métalloïdes)")
