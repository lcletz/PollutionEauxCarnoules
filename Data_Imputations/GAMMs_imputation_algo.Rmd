...
```{r}
safe_cowg <- na_cowg[(1:49),-3]
safe_cowg <- safe_cowg[which(colSums(is.na(safe_cowg))*100/nrow(safe_cowg)<=30)]
```
...
```{r}
safe_cowg <- safe_cowg[,-c(8:9,11:12,21:37,39:55,57:73,75:91)]
gam_res <- step_GAMMs(safe_cowg)
```
...
first_impute <- impute_GAMMs(safe_cowg, gam_res)
```
...
cowg_co <- impute_iter(first_impute, gam_res, max_iter = 200)
```

```{r}
#write_delim(data_complete, here("processed", "S1_gamm_2004_2008.csv"), delim=";")
write_delim(cowg_co, here("processed", "COWG_gamm_2004_2008.csv"), delim=";")
```
...
```{r}
#na_cowg <- na_cowg[-211,-c(3,6,12,13,15:21,24:26,28,30:32,35:37,39:42,44:54)]
na_cowg <- add_lag(na_cowg,4)
na_cowg <- na_cowg[,-c(17:29,31:43,45:57,59:71)]
#na_cowg <- na_cowg %>% select( names(cowg_co) )
```
...
cowg_co_long <- impute_byrow(na_cowg, cowg_co, gam_res)
```
...
n <- length(cowg_co_long[[2]])

for (col in 3:ncol(na_cowg)) {
  var <- list()

  for (i in 1:n) {
    if (names(na_cowg[col]) == cowg_co_long[[2]][[i]]) {
      var <- c(var, cowg_co_long[[2]][[i + 1]])
    }
  }

  var <- unlist(var)
  list_na <- append(list_na, list(var))
  list_var <- append(list_var, list(names(na_cowg[col])))
  list_final <- append(list_final, list(list_var, list_na))
}
...
```{r}
list_final <- c( list_final[[33]][1:13], list_final[[34]][1:13] )

new_order <- as.vector(rbind(1:16, 17:32))
reordered_list <- list_final[new_order]

new_impute <- list(cowg_co_long[[1]], reordered_list)
