...
library(lubridate)
library(imputeTS)
here::i_am("R/source_S1_desc_stat.Rmd")
library(here)
```

```{r}
S1 <- read.table(here('processed','Source_S1_v2.csv'), dec = c(',','.') , sep = ";", header = TRUE) %>% 
  mutate_at(2:45, function(x) as.numeric(as.character(x)))
...
```{r}
data_ts <- S1 %>%
  mutate_at(2:27, function(x) ts(x, start=c(2004,11), end=c(2018,6), frequency = 12))
complete_data <- data_ts %>%
  na_kalman(model = "auto.arima")
```

```{r}
write_delim(complete_data, here("processed","S1_ts.csv"), delim = ";")
```
...
```{r}
dis <- ts(data$monthly_discharge, start = c(2004,11), end = c(2018,6), frequency = 12)
