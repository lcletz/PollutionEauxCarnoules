...
library(lubridate)
library(imputeTS)
here::i_am("R/COWG_desc_stat.Rmd")
library(here)
```
# COWG MD imputation

```{r, warning=FALSE}
cowg <- read.table(here('processed','COWG.csv'), dec = c(',','.') , sep = ";", header = TRUE) %>% 
  mutate_at(2:45, function(x) as.numeric(as.character(gsub(',','.', x))))
cowg$Dates <- as.Date(cowg$Dates, format="%Y-%m-%d")
```

```{r}
recent_cowg <- read.table(here('processed','recent_COWG.csv'), dec = c(',','.'), sep = ";", 
                          header = TRUE) %>% 
  mutate_at(2:36, function(x) as.numeric(as.character(gsub(',','.', x))))
recent_cowg$Dates <- as.Date(recent_cowg$Dates, format="%Y-%m-%d")
```

```{r}
na_cowg <- cowg %>%
  bind_rows(recent_cowg)

na_cowg$Dates <- as.Date(substr(na_cowg$Dates, 1, 10))
all_months <- seq(
  from = as.Date(format(min(na_cowg$Dates), "%Y-%m-01")),
  to   = as.Date(format(max(na_cowg$Dates), "%Y-%m-01")),
  by = "month"
)
all_months_df <- data.frame(Dates = all_months)

na_cowg$MonthDate <- as.Date(format(na_cowg$Dates, "%Y-%m-01"))
na_cowg <- all_months_df %>%
  left_join(na_cowg, by = c("Dates" = "MonthDate")) %>%
  select(-Dates.y)

na_cowg <- na_cowg %>% select(which(colSums(is.na(na_cowg))<173))
```
...
```{r}
cowg_ts <- na_cowg %>%
  mutate_at(2:27, function(x) ts(x, start=c(2004,11), end=c(2022,5), frequency = 12))
complete_cowg <- cowg_ts %>%
  na_kalman(model = "auto.arima")
```

```{r}
ggplot_na_imputations(cowg_ts$Temperature, complete_cowg$Temperature)
```

```{r}
write_delim(complete_cowg, here("processed", "COWG_ts.csv"), delim = ";")
```
...
```{r}
data <- complete_cowg
rain <- ts(data$monthly_rainfall, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
temp <- ts(data$Temperature, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
pH <- ts(data$pH, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
Eh <- ts(data$Eh, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
O2 <- ts(data$O2, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
cond <- ts(data$Conductivité, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
lead <- ts(data$Plomb, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
alu <- ts(data$Aluminium, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
antimoine <- ts(data$Antimoine, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
arseniate <- ts(data$Arséniate, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
arsenite <- ts(data$Arsénite, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
arsenic <- ts(data$Arsenic_Total, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
baryum <- ts(data$Baryum, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
strontium <- ts(data$Strontium, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
sulfate <- ts(data$Sulfate, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
thallium <- ts(data$Thallium, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
zinc <- ts(data$Zinc, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
cadmium <- ts(data$Cadmium, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
chrome <- ts(data$Chrome, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
iron <- ts(data$Fer_Total, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
rub <- ts(data$Rubidium, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
cobalt <- ts(data$Cobalt, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
copper <- ts(data$Cuivre, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
nickel <- ts(data$Nickel, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
ion <- ts(data$Ion_Ferreux, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
dis <- ts(data$monthly_discharge, start = c(2004,11), end = c(2022,5), frequency = 12)
...
```{r}
library(htmlwidgets)
saveWidget(g, here("Réunions","cowg_interactive_plot.html"), selfcontained = TRUE)
```
