...
```{r, warning=FALSE}
gal <- read.table(here('processed','GAL.csv'), dec = c(',','.') , sep = ";", header = TRUE) %>% 
  mutate_at(2:45, function(x) as.numeric(as.character(gsub(',','.', x))))
gal$Dates <- as.Date(gal$Dates, format="%Y-%m-%d")
```

```{r}
gal$Dates <- as.Date(substr(gal$Dates, 1, 10))
all_months <- seq(
  from = as.Date(format(min(gal$Dates), "%Y-%m-01")),
  to   = as.Date(format(max(gal$Dates), "%Y-%m-01")),
  by = "month"
)
all_months_df <- data.frame(Dates = all_months)

gal$MonthDate <- as.Date(format(gal$Dates, "%Y-%m-01"))
gal <- all_months_df %>%
  left_join(gal, by = c("Dates" = "MonthDate")) %>%
  select(-Dates.y)

gal <- gal %>% select(which(colSums(is.na(gal))<121))
```
...
```{r}
ggplot_na_imputations(gal_ts$Temperature, complete_gal$Temperature)
```

```{r}
write_delim(complete_gal, here("processed", "gal_ts.csv"), delim = ";")
```
...
```{r}
library(htmlwidgets)
saveWidget(g, here("Réunions","gal_interactive_plot.html"), selfcontained = TRUE)
```
