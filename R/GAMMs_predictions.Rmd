...
```{r}
temp <- ts(data_complete$Temperature, start = c(2004,11), end = c(2018,6), frequency = 12)

fit_temp <- arima(temp, order = c(1, 1, 1),
      seasonal = list(order = c(1, 1, 1), period = 12), method = "CSS-ML")

pred_temp <- predict(fit_temp, n.ahead = 210)
```
...
ts <- ts(pred_temp$pred, start = c(2018,6), end = c(2026,6), frequency = 12)
ts_se <- ts(pred_temp$se, start = c(2018,6), end = c(2026,6), frequency = 12)
...
ts <- ts(pred_ph$pred, start = c(2008,12), end = c(2026,6), frequency = 12)
ts_se <- ts(pred_ph$se, start = c(2008,12), end = c(2026,6), frequency = 12)
x_coords <- c(time(ts), rev(time(ts)))
y_coords <- c(ts + ts_se, rev(ts - ts_se))
polygon(x_coords, y_coords, col = 'cyan3', border = "cyan")

points(ts, type='l', col='blue')
points(2025.33, new_data$cond, col="red", pch=19)
...
```{r}
ph <- ts(data_complete$pH, start = c(2004,11), end = c(2018,6), frequency = 12)
...
ts <- ts(pred_ph$pred, start = c(2018,6), end = c(2026,6), frequency = 12)
ts_se <- ts(pred_ph$se, start = c(2018,6), end = c(2026,6), frequency = 12)
x_coords <- c(time(ts), rev(time(ts)))
y_coords <- c(ts + ts_se, rev(ts - ts_se))
polygon(x_coords, y_coords, col = 'cyan3', border = "cyan")
...
```{r}
ars <- ts(data_complete$Arsenic_Total, start = c(2004,11), end = c(2018,6), frequency = 12)
...
ts <- ts(pred_ars$pred, start = c(2018,6), end = c(2026,6), frequency = 12)
ts_se <- ts(pred_ars$se, start = c(2018,6), end = c(2026,6), frequency = 12)
x_coords <- c(time(ts), rev(time(ts)))
y_coords <- c(ts + ts_se, rev(ts - ts_se))
polygon(x_coords, y_coords, col = 'cyan3', border = "cyan")

points(ts, type='l', col='blue')
points(ts(test$Arsenic_Total, start = c(2009,9), end = c(2019,1), frequency = 12),
       pch = 19, col = "darkred")
...
```{r}
data_small <- read.table(here('processed', 'COWG_gamm_2004_2008.csv'), dec = '.', sep = ';', header = T) 
data_small$Dates <- as.Date(data_small$Dates, format = "%Y-%m-%d")
```

```{r}
data_complete <- read.table(here('processed', 'COWG_gamm_2004_2018.csv'), dec = '.', sep = ';', header = T)
data_complete$Dates <- as.Date(data_complete$Dates, format = "%Y-%m-%d")
```
...
```{r}
temp_small <- ts(data_small$Temperature, start = c(2004,11), end = c(2008,11), frequency = 12)
temp_complete <- ts(data_complete$Temperature, start = c(2004,11), end = c(2022,4), frequency = 12)

fit_temp_small <- auto.arima(temp_small, max.p = 1, max.q = 1, ic = "bic")
fit_temp_complete <- auto.arima(temp_complete, max.p = 1, max.q = 1, ic = "bic")

pred_temp_small <- predict(fit_temp_small, n.ahead = 210)
pred_temp_complete <- predict(fit_temp_complete, n.ahead = 40)
```
...
We can almost predict temperature with the completely imputed dataset. Let's keep that in mind if we find a variable we can't predict with the small dataset.
...
test <- read.table(here('processed','COWG.csv'), dec = c(',','.'), sep = ";", header = TRUE)  %>%
  mutate_at(2:45, function(x) as.numeric(as.character(gsub(',','.',x))))
test$Dates <- as.Date(test$Dates, format="%Y-%m-%d")

test$Dates <- as.Date(substr(test$Dates, 1, 10))
all_months <- seq(
  from = as.Date(format(min(test$Dates), "%Y-%m-01")),
  to   = as.Date(format(max(test$Dates), "%Y-%m-01")),
  by = "month"
)
all_months_df <- data.frame(Dates = all_months)

test$MonthDate <- as.Date(format(test$Dates, "%Y-%m-01"))
test <- all_months_df %>%
  left_join(test, by = c("Dates" = "MonthDate"))
```
...
for(i in 1:4){
  ...
  new_col <- new_col[ 1:(length(new_col)-i) ]
  new_col <- c( rep(as.numeric(NA), i), new_col)
  name_col <- paste0( names(test[2]), "_lag_", i )
  test <- test %>%
    bind_cols( new_col )
colnames( test )[ncol(test)] <- name_col }
...
points(pred_temp_small$pred, type='l', col='blue')
points(2025.33, new_data$Temperature, col="red", pch=19)
points(ts(test$Temperature, start = c(2009,9), end = c(2019,1), frequency = 12),
       pch = 19, col = "darkred")
...
```{r}
temp <- ts(data_complete$Temperature, start = c(2004,11), end = c(2022,4), frequency = 12)

fit_temp <- arima(temp, order = c(2, 1, 2),
      seasonal = list(order = c(1, 1, 2), period = 12), method = "CSS-ML")

pred_temp <- predict(fit_temp, n.ahead = 50)
```

```{r}
plot.ts(temp, type='l', xlab='Temps', ylab="Température (en C°)",
        xlim = c(2004,2026), xaxt = 'n')

ts <- ts(pred_temp$pred, start = c(2022,4), end = c(2026,6), frequency = 12)
...
```{r}
plot.ts(o2, type='l', xlab='Temps', ylab="O2",
        xlim = c(2004,2026), ylim = c(-4,15), xaxt = 'n')

ts <- ts(pred_o2$pred, start = c(2008,12), end = c(2025,6), frequency = 12)
ts_se <- ts(pred_o2$se, start = c(2008,12), end = c(2025,6), frequency = 12)
x_coords <- c(time(ts), rev(time(ts)))
y_coords <- c(ts + ts_se, rev(ts - ts_se))
polygon(x_coords, y_coords, col = 'cyan3', border = "cyan")
...
points(ts, type='l', col='blue')
points(2025.33, new_data$O2, col="red", pch=19)
points(ts(test$O2, start = c(2009,9), end = c(2019,1), frequency = 12),
       pch = 19, col = "darkred")
...
ts <- ts(pred_ph$pred, start = c(2008,12), end = c(2025,6), frequency = 12)
ts_se <- ts(pred_ph$se, start = c(2008,12), end = c(2025,6), frequency = 12)
x_coords <- c(time(ts), rev(time(ts)))
y_coords <- c(ts + ts_se, rev(ts - ts_se))
polygon(x_coords, y_coords, col = 'cyan3', border = "cyan")
...
```{r}
ph <- ts(data_complete$pH, start = c(2004,11), end = c(2022,4), frequency = 12)

#fit_ph <- auto.arima(ph, max.p = 3, max.q = 3, ic = "aic")
fit_ph <- arima(ph, order = c(2, 1, 2),
      seasonal = list(order = c(1, 1, 2), period = 12), method = "CSS-ML")

pred_ph <- predict(fit_ph, n.ahead = 50)
```
...
```{r}
ars <- ts(data_complete$Arsenic_Total, start = c(2004,11), end = c(2022,6), frequency = 12)

#fit_ars <- auto.arima(ars, max.p = 3, max.q = 3, ic = "aic")
fit_ars <- arima(ars, order = c(1, 1, 1),
      seasonal = list(order = c(1, 1, 1), period = 12), method = "CSS-ML")

pred_ars <- predict(fit_ars, n.ahead = 150)
```
...
ts <- ts(pred_ars$pred, start = c(2022,6), end = c(2026,6), frequency = 12)
ts_se <- ts(pred_ars$se, start = c(2022,6), end = c(2026,6), frequency = 12)
x_coords <- c(time(ts), rev(time(ts)))
y_coords <- c(ts + ts_se, rev(ts - ts_se))
polygon(x_coords, y_coords, col = 'cyan3', border = "cyan")
...
```{r}
lead <- ts(data_complete$Plomb, start = c(2004,11), end = c(2022,6), frequency = 12)

fit_lead <- arima(lead, order = c(1, 1, 1),
      seasonal = list(order = c(1, 1, 1), period = 12), method = "CSS-ML")

pred_lead <- predict(fit_lead, n.ahead = 150)
```
...
ts <- ts(pred_lead$pred, start = c(2022,6), end = c(2026,6), frequency = 12)
ts_se <- ts(pred_lead$se, start = c(2022,6), end = c(2026,6), frequency = 12)
x_coords <- c(time(ts), rev(time(ts)))
y_coords <- c(ts + ts_se, rev(ts - ts_se))
polygon(x_coords, y_coords, col = 'cyan3', border = "cyan")
