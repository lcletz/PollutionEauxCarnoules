...
```{r, warning=FALSE}
S5 <- read.table(here('processed','S5.csv'), dec = c(',','.') , sep = ";", header = TRUE) %>% 
  mutate_at(2:45, function(x) as.numeric(as.character(gsub(',','.', x))))
S5$Dates <- as.Date(S5$Dates, format="%Y-%m-%d")
```

```{r}
S5$Dates <- as.Date(substr(S5$Dates, 1, 10))
all_months <- seq(
  from = as.Date(format(min(S5$Dates), "%Y-%m-01")),
  to   = as.Date(format(max(S5$Dates), "%Y-%m-01")),
  by = "month"
)
all_months_df <- data.frame(Dates = all_months)

S5$MonthDate <- as.Date(format(S5$Dates, "%Y-%m-01"))
S5 <- all_months_df %>%
  left_join(S5, by = c("Dates" = "MonthDate")) %>%
  select(-Dates.y)

S5 <- S5 %>% select(which(colSums(is.na(S5))<133))
```
...
```{r}
S5_ts <- S5 %>%
  mutate_at(2:24, function(x) ts(x, start=c(2004,11), end=c(2019,1), frequency = 12))
complete_S5 <- S5_ts %>%
  na_kalman(model = "auto.arima")
```

```{r}
ggplot_na_imputations(S5_ts$Temperature, complete_S5$Temperature)
```

```{r}
write_delim(complete_S5, here("processed", "S5_ts.csv"), delim = ";")
```
...
```{r}
rub <- ts(data$Rubidium, start = c(2004,11), end = c(2019,1), frequency = 12)
...
```{r}
library(htmlwidgets)
saveWidget(g, here("Réunions","S5_interactive_plot.html"), selfcontained = TRUE)
```
